...
This fork's x86_64 automation checks for a new published upstream release every six hours, safely merges its tag into `x86_64-actions`, and starts the build only when the matching x86_64 Release is absent. The build works around the six-hour GitHub-hosted job limit by running up to four bounded compiler-cache warmup jobs followed by one build/signing job. Every stage stops after 300 minutes so setup, cache upload and cleanup remain below the hard job limit. If a warmup completes the target, it immediately signs and publishes the APK and SHA256 checksum, and the remaining stages are skipped. If the final stage is still incomplete, it saves its ccache checkpoint and then fails deliberately; a later run continues from that checkpoint instead of losing the whole attempt. Cache lookup also falls back across CI-only commits, while ccache still validates compiler inputs before every hit.
